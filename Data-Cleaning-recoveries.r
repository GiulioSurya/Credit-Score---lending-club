...
#RECOVERIES HAS A CORRELATION HIGHER THAN 0.85 WITH VAR NUMBER 2 3 4 20, I WILL TRY TO REMOVE THEM
valiables_rem_nam<-names(dati[,sapply(dati,is.numeric)])[c(2,3,4,20)]
dati<-dati[,!names(dati) %in% valiables_rem_nam]
}

{

dati<-subset(dati, select=-c(X,id,loan_amnt,funded_amnt_inv,issue_d,grade,sub_grade,emp_title,url,
...
dati<-subset(dati, select = -c(collection_recovery_fee))
}
...
#step wise forward selection
stepwise_fit <- regsubsets(recoveries ~ ., data = dati[train,], nvmax = 200, really.big = TRUE)
...
total_rec_int+total_rec_late_fee+total_bal_il, data = dati[train,])
summary(lm_fit)
prova<-cbind(best_model_predictors,dati$recoveries,dati$funded_amnt)
#correlation matrix for the best mode
dati_cor<-dati[,best_model_predictors]
test1<-cbind(dati_cor,dati$recoveries,dati$funded_amnt)
M <- cor(test1,use="complete.obs")
corrplot(M, type="upper",method="number",number.cex=0.5,addCoef.col = "black",tl.cex = 0.5)
