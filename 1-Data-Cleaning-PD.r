#########DATA CLEANING AND EDA FOR PROBABILTY OF DEFAULT
remove(list = ls())
{
library(tidyverse)
library(leaps)
...
#########CLEANING
{
  #removing variables with significant amount of NA
  dati<-data[,colMeans(is.na(data))<0.5] 
  #removing  irrelevant variables for our analysis
...
#########EDA
prop.table(table(dati$default))
#->class imbalance: the class labeled as '0' constitutes 88% of the dataset

#analize plot of all variables:
{
#INCOME
plot(dati$annual_inc)
tail(sort(dati$annual_inc)) #extreme values
...
#Break income into classes to see how default is distributed among them:
dati$annual_inc_class<-cut(dati$annual_inc, 
                           breaks = c(0,50000,100000,150000,200000,250000,300000,350000,
                                      400000,450000,500000,550000,600000,650000,700000,750000,Inf), 
                           labels = c("0-50k","50-100k","100-150k","150-200k","200-250k","250-300k",
                                      "300-350k","350-400k","400-450k","450-500k","500-550k","550-600k",
                                      "600-650k","650-700k","700-750k","750k+"))
table(dati$annual_inc_class, dati$default)
prop.table(table(dati$annual_inc_class, dati$default), 1)
#-> Similar default rates among all the income classes

#FICO: it's a credit score. Higher value means better credit score.
...
summary(dati$installment)
table(cut(dati$installment, 
                     breaks = c(0,100,250,500,750,1000,1250,1500,Inf)), dati$default)
prop.table(table(cut(dati$installment, 
                     breaks = c(0,100,250,500,750,1000,1250,1500,Inf)), dati$default),1)

#GRADE: credit score. Higher value (close to A) means better credit score.
table(dati$grade, dati$default)
prop.table(table(dati$grade, dati$default), 1)
#-> more default in low grade

#HOME OWNERSHIP
table(dati$home_ownership, dati$default)
dati <- subset(dati, home_ownership != "NONE") #delete 3 observation with home_ownership=="NONE"
dati$home_ownership <- droplevels(dati$home_ownership) #remove unused levels from the home_ownership factor
prop.table(table(dati$home_ownership, dati$default), 1)
#more or less same default among categories

#Analyzing default vs income among individuals with home_ownership=="MORTGAGE"
...
#->same default rates among emp_length
...
#->"small business" has the highest number of default
...
#breaks the variable in classes
...
#DEBT_SETTLEMENT_FLAG: whether or not the borrower, who has charged-off, is working with a debt-settlement company.
...
#basically all the observation with default=1 have flag 
}

#create a dataset that we will use to build the model
